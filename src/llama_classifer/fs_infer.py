...
# === Config ===pheme_merged_output.csv

model_path = "../llama-3.1-8b-hf/"

input_csv = "pheme_merged_output.csv"

output_csv = "predictions_llama_pheme_zs_first_draft.csv"
...
real_shots = df[df['label'].astype(str).str.lower() == 'true'].sample(num_shots // 2)

fake_shots = df[df['label'].astype(str).str.lower() == 'false'].sample(num_shots // 2)
...
for idx, row in df.iterrows():
  ...
  # output = output.lower()
   ...
   # match = re.search(r"\b(true|false)\b", output)
    ...
    match = re.search(r"\b(false|true)\b", decoded.lower())
    ...
    if idx % 50 == 0 and idx > 0:
        ...
        pd.DataFrame(predictions).to_csv("predictions_partial_pheme.csv", index=False)
